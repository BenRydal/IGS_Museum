function mouseMoved() {
    ...
    // Individual buttons (15 colored lines at top)
    var individualButtonSpacing, i;
    for (i = 0; i < individualLength; i++) {
        if (i == 0 || i == 5 || i == 9 || i == 11) {
            findFamily(i);
            individualButtonSpacing = 0;
        }
    ...
    // Mode buttons (Movement / Talk / Curation)
    if (overRect(xPosMapMovementButton, yPosMapButton, xPosMapCurationButtonEnd, mapButtonSizeHeight)) {
        cursor(HAND);
        return;
    }

    if (zoomView) {
        // Animation on/off & About buttons
        textSize(18);
        ...
        if (overRect(introMsgButtonXPos, introButtonTop, introMsgButtonXPos + a + b, introButtonBottom)) {
            cursor(HAND);
            return;
        }

        // Family buttons (4 circles)
        if (dist(mouseX, mouseY, zoomFamilyX1, zoomFamilyY) < mapButtonSize ||
            dist(mouseX, mouseY, zoomFamilyX2, zoomFamilyY) < mapButtonSize ||
            dist(mouseX, mouseY, zoomFamilyX3, zoomFamilyY) < mapButtonSize ||
            dist(mouseX, mouseY, zoomFamilyX4, zoomFamilyY) < mapButtonSize) {
            cursor(HAND);
            return;
        }

        // Space buttons (3 circles)
        if (dist(mouseX, mouseY, zoomSpaceX, zoomSpaceY1) < mapButtonSize ||
            dist(mouseX, mouseY, zoomSpaceX, zoomSpaceY2) < mapButtonSize ||
            dist(mouseX, mouseY, zoomSpaceX, zoomSpaceY3) < mapButtonSize) {
            cursor(HAND);
            return;
        }
        ...
        // Zoom buttons (small multiple view, 11 circles)
        var zoomX, zoomY;
        for (i = 0; i < 11; i++) {
        ...
        if (talk && overRect(xPosReset, yPosReset + resetHeight + 1, xPosReset + conversationKeyWidth, yPosReset + 2 * resetHeight)) {
            cursor(HAND);
            return;
        }
    }
...
function mousePressed() {
    if (welcome && !window._igsSvelteWelcome) {
        welcome = false;
...
function overIntroButtons() {
    ...
    if (overRect(introMsgButtonXPos, introButtonTop, introMsgButtonXPos + a, introButtonBottom)) {
        animate = !animate;
        reveal = 0;
    } else if (overRect(introMsgButtonXPos + a, introButtonTop, introMsgButtonXPos + a + b, introButtonBottom)) {
        welcome = !welcome;
        document.dispatchEvent(new CustomEvent('igsWelcomeToggle', { detail: { visible: welcome } }));
    }
}
...
function overRect(x, y, sizeWidth, sizeHeight) {
    return mouseX >= x && mouseX <= sizeWidth && mouseY >= y && mouseY <= sizeHeight;
}
